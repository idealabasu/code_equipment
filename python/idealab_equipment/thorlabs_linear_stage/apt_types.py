...
class APTData(object):
    ...
    def get_bit(self,num):
        test = (1<<num)
        return (self.value & test)==test


class APTWord(APTData):
    num_bytes = 2

class APTDWord(APTData):
    num_bytes = 4

class APTByte(APTData):
    num_bytes = 1
class APTByte16(APTData):
    num_bytes = 16
class APTByte48(APTData):
    num_bytes = 48
...
class APTChar(APTData):
    num_bytes = 1
    
    def __str__(self):
        return self.value.decode()

class APTChar8(APTChar):
    num_bytes = 8

class APTChar16(APTChar):
    num_bytes = 16

class APTChar64(APTChar):
    num_bytes = 64
