...
des_pos = numpy.zeros([sample_num,len(initial_pose)])

# des_pos[0:half_sample_num,0] = initial_pose[0] + numpy.linspace(0,forward_dis,half_sample_num)
# des_pos[0:half_sample_num,1] = initial_pose[1] 
# des_pos[0:half_sample_num,2] = initial_pose[2] + numpy.sin(numpy.linspace(0,forward_dis,half_sample_num)*(2*pi/(2*forward_dis)))*0.05

# des_pos[half_sample_num:,0] = des_pos[half_sample_num-1,0] - numpy.linspace(0,forward_dis,half_sample_num)
# des_pos[half_sample_num:,1] = des_pos[half_sample_num-1,1]
# des_pos[half_sample_num:,2] = des_pos[half_sample_num-1,2] - numpy.sin(numpy.linspace(0,forward_dis,half_sample_num)*(2*pi/(2*forward_dis)))*0.1

des_pos[0:half_sample_num,0] = numpy.linspace(0,forward_dis,half_sample_num)
des_pos[0:half_sample_num,1] = 0
des_pos[0:half_sample_num,2] = numpy.sin(numpy.linspace(0,forward_dis,half_sample_num)*(2*pi/(2*forward_dis)))*0.05
...
des_pos_trans = numpy.matmul(numpy.asanyarray(r.as_matrix()),des_pos[:,0:3].T).T

des_pos[:,0:3] = des_pos_trans+initial_pose[0:3]

des_pos[:,3] = initial_pose[3]
des_pos[:,4] = initial_pose[4]
des_pos[:,5] = initial_pose[5]
diff_steps = des_pos[1:,:] - des_pos[0:-1:]

# plt.plot(des_pos[:,0],des_pos[:,2])

current_pose = ur5.get_pos()[:]
ur5.movels(des_pos,vel=0.05,acc=1,radius=0.005, wait=False)
while True:
    current_pose = numpy.vstack((current_pose,ur5.get_pos()[:]))   
plt.plot(current_pose)
plt.figrue()

# plt.plot(current_pose[:,0]-current_pose[0,0],current_pose[:,2]-current_pose[0,2])
fig = plt.figure()
ax = plt.axes(projection='3d')
ax.plot(current_pose[:,0],current_pose[:,1],current_pose[:,2])
ax.plot(des_pos[:,0],des_pos[:,1],des_pos[:,2],'--')
