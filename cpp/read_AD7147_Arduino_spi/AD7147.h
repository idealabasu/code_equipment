...
#define MAX_SLIDER_VAL			255
#define VALS_PER_PAD			(MAX_SLIDER_VAL / NUMBER_OF_SENSORS)
#define ACTIVATION_THRESHOLD	1000
...
//    void readtest(const uint16_t RegisterStartAddress, const byte NumberOfRegisters, uint16_t DataBuffer, const uint16_t OffsetInBuffer);
    ...
    void  write(const uint16_t RegisterAddress, const byte NumberOfRegisters, uint16_t *DataBuffer, const byte OffsetInBuffer);
    void  read(const uint16_t RegisterStartAddress, const byte NumberOfRegisters, uint16_t *DataBuffer, const uint16_t OffsetInBuffer);

    void init(uint8_t a);
    uint16_t  SliderPosition1;
  private:
    uint16_t AD7147Registers[NUMBER_OF_AD7147_REGISTERS];
    int CS_PIN, INT_PIN;

    //SLIDER
    uint16_t  AmbientValues[NUMBER_OF_SENSORS];
    uint16_t  SensorValues[NUMBER_OF_SENSORS];
    float	  SliderBins[NUMBER_OF_SENSORS];

    byte InterruptCounterForInitialisation = 0;
    byte InterruptCounterForThresIntMode = 0;

    //Position variables
    uint16_t  SliderPosition;

    void readAllSensors(void);
    void setAmbientValues(void);
