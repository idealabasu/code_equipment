...
uint16_t AD7147::readAllSensors1() {
  ...
  for (i = 0; i < NUMBER_OF_SENSORS; i++)
  {
    if (AD7147Registers[ADCRESULT_S0 + i] > AmbientValues[i])
      ...
      SensorValues[i] = 0;
      Serial.print("ADC/AMBIENNT/Sensor Value ");
      Serial.print(i);
      Serial.print(" : ");
      Serial.print(AD7147Registers[ADCRESULT_S0 + i]);
      Serial.print(" + ");
      Serial.print(AmbientValues[i]);
      Serial.print(" + ");
...
//void  AD7147::readtest(RegisterStartAddress,NumberOfRegisters,DataBuffer,OffsetInBuffer){
...
void AD7147::read(const uint16_t RegisterStartAddress, const byte NumberOfRegisters, uint16_t *DataBuffer, const uint16_t OffsetInBuffer)
{
  ...
  Serial.println(*(DataBuffer + OffsetInBuffer + RegisterIndex));
  }  
  SPI.endTransaction();
  digitalWrite(CS_PIN, HIGH);
}
